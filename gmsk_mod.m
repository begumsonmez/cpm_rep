...
% Sampling frequency is selected such that we over sample by a factor of L of the carrier frequency

a=a(:); %serialize input stream
ck = 2*a-1;%NRZ format
ct = kron(ck,ones(L,1));%Convert to waveform using oversampling
...
%gt = gt(delay+1:end-delay);%remove unwanted portions from output
bt = bt/max(abs(bt));%normalize the output of Gaussian LPF to +/-1

phi = filter(1,[1,-1],bt*Ts);
phi = phi *0.5*pi/Tb; %h=0.5 - %integrate to get phase information %the constant in the diagram before the I/Q Modulator

I = cos(phi); Q = sin(phi); %cross-correlated baseband I/Q signals
s_complex = I - 1i*Q; %complex baseband representation - simulation kodunda sinyalin enerjisini hesaplamak için kullanýyoruz, cepte
t=((0:1:length(I)-1)*Ts).'; %for RF carrier
iChannel = I.*cos(2*pi*Fc*t); qChannel = Q.*sin(2*pi*Fc*t);
s =(iChannel - qChannel).'; %real signal - with RF carrier %This is the signal to be transmitted
doPlot=1;

if doPlot==1, figure;
subplot(2,4,1);plot((0:1:length(ct)-1)*Ts,ct); xlim([0,10*Tb]); title('c(t)');
subplot(2,4,2);plot(-k*Tb:Ts:k*Tb,ht);title(['h(t)-BT_b=',num2str(BT)]);
subplot(2,4,5);plot((0:1:length(bt)-1)*Ts,bt); xlim([0,10*Tb]); title('b(t)');
